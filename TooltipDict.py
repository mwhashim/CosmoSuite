...
#---------------------------
gadget_make_tooltip = OrderedDict([
...
(' - PEANOHILBERT:',
     'This is a tuning option. When set, the code will bring the particles after each domain decomposition into Peano-Hilbert order. This improves cache utilization and performance.'),
...
(' - DOUBLEPRECISION:',
     'This makes the code store and compute internal particle data in double precision. Note that output files are nevertheless written by converting the particle data to single precision.'),
...
(' - SYNCHRONIZATION:',
     'When this is set, particles are kept in a binary hierarchy of timesteps and may only increase their timestep if the new timestep will put them into synchronization with the higher time level.'),
...
(' - NOPMSTEPADJUSTMENT:',
     'When this is set, the long-range timestep for the PM-force computation (when the TreePM algorithm is used) is always determined by MaxSizeTimeStep.  Otherwise, it is determined by the MaxRMSDisplacement parameter, or MaxSizeTimeStep, whichever gives the smaller step.'),
...
(' - OUTPUTPOTENTIAL:',
     'This will make the code compute gravitational potentials for all particles each time a snapshot file is generated. The values are then included in the snapshot file. Note that the computation of the values of the gravitational potential costs additional CPU.'),
...
(' - OUTPUTTIMESTEP:',
     'This will include the current timesteps of all particles in the snapshot files.'),

(' - NOGRAVITY',
     'This switches off gravity. Useful only for pure SPH simulations in non-expanding space.'),
...
(' - NOTYPEPREFIX_FFTW:',
     'This is an option that signals that FFTW has been compiled without the type-prefix option, i.e. no leading "d" or "s" characters are used to access the library.'),

(' - LONG_X:',
     'These options can be used together with PERIODIC and NOGRAVITY only. When set, the options define numerical factors that can be used to distorts the periodic simulation cube into a parallelepiped of arbitrary aspect ratio. This can be useful for idealized SPH tests.'),
...
(' - NOVISCOSITYLIMITER:',
     "If this is set, the code will not try to put an upper limit on the viscous force in case an implausibly high pair-wise viscous force (which may lead to a particle 'reflection' in case of poor timestepping) should arise. Note: For proper settings of the timestep parameters, this situation should not arise."),
...
(' - FORCETEST:',
     'This can be set to check the force accuracy of the code. The option needs to be set to a number between 0 and 1 (e.g. 0.01), which is taken to specify a random fraction of particles for which at each timestep forces by direct summation are computed. The normal tree-forces and the correct direct summation forces are collected in a file. Note that the simulation itself is unaffected by this option, but it will of course run much(!) slower, especially if FORCETEST*NumPart*NumPart >> NumPart. Note: Particle IDs must be set to numbers >=1 for this to work.'),
...
#--------------------------------
N_GenIC_make_tooltip = OrderedDict([
(' -DPRODUCEGAS',
    'Set this to automatically produce gas particles. For a single DM species in the input file by interleaved by a half a grid spacing. Only for Gaussian and ZA '),

('-DMULTICOMPONENTGLASSFILE',
    'Set this if the initial glass file contains multiple components. Only for Gaussian and ZA'),

('-DDIFFERENT_TRANSFER_FUNC',
    'Set this if you want to implement a transfer function that depends on particle type. Only for Gaussian and ZA'),

('-DNO64BITID',
    'Switch this on if you want normal 32-bit IDs'),

('-DCORRECT_CIC',
    'Only switch this on if particles start from a glass (as opposed to grid). Only for Gaussian and ZA'),

('-DONLY_ZA',
    'Swith this on if you want ZA initial conditions (2LPT otherwise)'),

('-DONLY_GAUSSIAN',
    'Shwich this if you want gaussian initial conditions (fnl otherwise)'),

('-DLOCAL_FNL',
    'Switch this if you want only local non-gaussianities'),

('-DEQUIL_FNL',
    'Switch this if you want equilateral Fnl'),

('-DORTOG_FNL',
    'Switch this if you want ortogonal Fnl') ])
